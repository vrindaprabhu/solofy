...
class DetectionLayer(nn.Module):
    ...
    def forward(self, x, cuda):
        ...
        anchors = (
            self.anchors.unsqueeze(-1)
            .unsqueeze(-1)
            .expand_as(detection[:, :, 2:4, :, :])
        )
        if cuda:
            anchors = anchors.cuda()

        detection[:, :, 2:4, :, :] = (
            torch.exp(detection[:, :, 2:4, :, :]) * anchors
        )
        detection = (
            detection.transpose(1, 2)
            .contiguous()
            .view(batch_size, self.num_classes + 5, -1)
            .transpose(1, 2)
        )
...
def create_modules(blocks):
    ...
    out_channels = (
        []
    )  # keep track of output channel for every block for specifying conv layer input channels
    ...
    for i, block in enumerate(blocks[1:]):
        ...
        if block_type == "convolutional":
            ...
            if bn:
                module.add_module(
                    "batchnorm_%d" % (i), nn.BatchNorm2d(filters)
                )
            if activation == "leaky":
                module.add_module(
                    "leaky_%d" % i, nn.LeakyReLU(0.1, inplace=True)
                )
        ...
        # route block could have one or two indices. Negative value means relative index.
            ...
            if len(layer_indices) > 1:
                ...
                if second_idx < 0:
                    second_idx += i
                out_channel = (
                    out_channels[first_idx] + out_channels[second_idx]
                )
            ...
            anchors = [
                [int(anchors[2 * i]), int(anchors[2 * i + 1])] for i in masks
            ]
