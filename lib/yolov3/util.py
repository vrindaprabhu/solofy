...
def process_result(detection, obj_threshhold, nms_threshhold):
    ...
    for batchi in range(detection.size(0)):
        ...
        pred_classes = torch.unique(bboxes[:, -1].data)
        ...
        for cls in pred_classes:
            ...
            batch_idx_add = torch.full(
                (bboxes_cls.size(0), 1), batchi, dtype=torch.float32
            )
...
def compute_ious(target_box, comp_boxes):
    ...
    intercept_areas = torch.clamp(
        interceptx2s - interceptx1s + 1, 0
...
def resize_image(img, size):
    ...
    canvas[
        (newh - img_h) // 2 : (newh - img_h) // 2 + img_h,
        (neww - img_w) // 2 : (neww - img_w) // 2 + img_w,
        :,
    ] = img
...
def transform_result(detections, imgs, input_size):
    ...
    detections[:, [1, 3]] -= (
        input_size[1] - scale_factors * img_dims[:, 1].unsqueeze(-1)
    ) / 2
    detections[:, [2, 4]] -= (
        input_size[0] - scale_factors * img_dims[:, 0].unsqueeze(-1)
    ) / 2
